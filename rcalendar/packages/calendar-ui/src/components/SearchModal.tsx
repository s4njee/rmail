...
export const SearchModal: Component<SearchModalProps> = (props) => {
  ...
  return (
    <Show when={props.isOpen}>
      ...
      >
        {/* Search Input Bar */}
        <div style={{ display: 'flex', 'align-items': 'center', gap: '12px', padding: '16px 20px', 'border-bottom': '1px solid var(--al-grid, #EBEBEB)' }}>
          <div style={{ width: '14px', height: '14px', border: '2px solid var(--al-ink-7, #A0A0A0)', 'border-radius': '50%', flex: 'none' }} />
          <input
            type="text"
          ...
          />
          <span style={{ 'font-family': 'var(--al-font-mono)', 'font-size': '11px', color: 'var(--al-ink-7, #A0A0A0)' }}>
            ESC
          </span>
        </div>
        ...
        <div style={{ padding: '16px 20px', 'overflow-y': 'auto', display: 'flex', 'flex-direction': 'column', gap: '16px' }}>
          ...
          <Show when={results().matchedDate}>
            <div>
              <div style={{ 'font-family': 'var(--al-font-mono)', 'font-size': '9.5px', 'letter-spacing': '0.12em', color: 'var(--al-ink-7, #A0A0A0)', 'margin-bottom': '8px' }}>
                JUMP TO DATE
              </div>
              <button
                type="button"
              ...
              <div style={{ display: 'flex', 'flex-direction': 'column', gap: '4px' }}>
                <For each={results().events}>
                  {(evt) => {
                    ...
                    const dateText = start.toLocaleDateString(undefined, { month: 'short', day: 'numeric' });
                    return (
                      <button
                        type="button"
                        onClick={() => {
                      ...
                      >
                        <div style={{ width: '3px', height: '14px', 'border-radius': '2px', background: 'var(--al-accent, #1F6FEB)', flex: 'none' }} />
                        <span style={{ 'font-size': '13px', color: 'var(--al-ink, #1A1A1A)', flex: 1 }}>
                          {evt.title}
                        </span>
                        <span style={{ 'font-family': 'var(--al-font-mono)', 'font-size': '11px', color: 'var(--al-ink-7, #A0A0A0)' }}>
                          {dateText}
                ...
                <For each={results().tasks}>
                  {(task) => (
                    <div style={{ display: 'flex', 'align-items': 'center', gap: '10px', padding: '6px 12px' }}>
                      <div style={{ width: '10px', height: '10px', 'border-radius': '50%', border: '1.5px solid var(--al-cal-classes, #C2410C)', flex: 'none' }} />
                      <span style={{ 'font-size': '13px', color: 'var(--al-ink, #1A1A1A)' }}>{task.title}</span>
                    </div>
                  )}
                </For>
              </div>
          ...
          <Show when={query().trim() && !loading() && results().events.length === 0 && results().tasks.length === 0 && !results().matchedDate}>
            <div style={{ padding: '24px 0', 'text-align': 'center', color: 'var(--al-ink-7, #A0A0A0)', 'font-size': '13px' }}>
