...
export const WeekView: Component<WeekViewProps> = (props) => {
  ...
  return (
    ...
    >
      ...
      {/* Column Headers (52px) */}
      <div style={{ height: '52px', flex: 'none', display: 'flex', 'border-bottom': '1px solid var(--al-border-soft, #E5E5E5)' }}>
        <div style={{ width: '62px', flex: 'none', 'border-right': '1px solid var(--al-grid, #EBEBEB)' }} />
        <For each={weekDays()}>
          {(day, i) => {
            const today = new Date();
      ...
      {/* All-Day Band (40px) */}
      <div style={{ height: '40px', flex: 'none', display: 'flex', 'border-bottom': '1px solid var(--al-border-soft, #E5E5E5)', background: 'var(--al-surface-3, #FCFCFC)' }}>
        <div
          style={{
            width: '62px',
            flex: 'none',
        ...
        <For each={weekDays()}>
          {(day) => {
            ...
            return (
              <div style={{ flex: 1, 'min-width': 0, 'border-right': '1px solid var(--al-grid, #EBEBEB)', padding: '5px 4px', display: 'flex', 'flex-direction': 'column', 'justify-content': 'center', gap: '2px', overflow: 'hidden' }}>
                <For each={events()}>
                  {(item) => {
      ...
      <div style={{ flex: 1, display: 'flex', 'min-height': 0, position: 'relative', 'overflow-y': 'auto' }}>
        {/* Hour Gutter */}
        <div style={{ width: '62px', flex: 'none', 'border-right': '1px solid var(--al-grid, #EBEBEB)', position: 'relative' }}>
          <For each={hoursList()}>
            {(h) => (
              <div style={{ height: `${GRID_CONFIG.rowPitch}px`, display: 'flex', 'align-items': 'flex-start', 'justify-content': 'flex-end', 'padding-right': '9px' }}>
                <span style={{ 'font-family': 'var(--al-font-mono)', 'font-size': '10px', color: 'var(--al-ink-8, #B3B3B3)', transform: 'translateY(-5px)' }}>
                  {h.label}
                </span>
              </div>
            )}
                ...
                <For each={dayEvents()}>
                  {(pe) => {
                    ...
                    return (
                      ...
                      >
                        <span style={{ 'font-size': '11.5px', 'font-weight': 500, 'line-height': 1.25, color: 'var(--al-ink-event, #232323)', 'white-space': 'nowrap', overflow: 'hidden', 'text-overflow': 'ellipsis' }}>
                          {pe.item.event.title}
                        </span>
                        <span style={{ 'font-family': 'var(--al-font-mono)', 'font-size': '9.5px', color: color() }}>
                          {timeStr()}
                        </span>
