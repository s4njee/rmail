...
import { computeNowLinePosition, GridConfig, positionEventsForDay } from "../headless/layout";
import {
...
export const ThreeDayView: Component<ThreeDayViewProps> = (props) => {
  ...
  const threeDays = createMemo(() => {
    return [props.focusedDate, addDays(props.focusedDate, 1), addDays(props.focusedDate, 2)];
  });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          style={{
            width: props.showSecondaryTz && props.secondaryTz ? "92px" : "70px",
            flex: "none",
            "border-right": "1px solid var(--al-grid, #EBEBEB)",
          }}
        >
                ...
                <For each={dayEvents()}>
                  {(pe) => {
                    ...
                    const effectiveHeight = () => {
                      if (isBeingResized())
                        return Math.max(24, pe.height + (resizingItem()?.deltaY || 0));
                      return pe.height;
                    };
