...
export const SettingsView: Component<SettingsViewProps> = (props) => {
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ display: 'flex', 'align-items': 'center', 'justify-content': 'space-between', padding: '0 10px 12px' }}>
          <span style={{ 'font-family': 'var(--al-font-mono)', 'font-size': '9.5px', 'letter-spacing': '0.12em', color: 'var(--al-ink-7, #A0A0A0)' }}>
            SETTINGS
          </span>
          <button
            type="button"
        ...
        <div style={{ padding: '0 10px', display: 'flex', 'flex-direction': 'column', gap: '3px' }}>
          ...
          <span style={{ 'font-family': 'var(--al-font-mono)', 'font-size': '10px', color: '#BFBFBF' }}>
            tauri 2.4 · sqlite
          </span>
        </div>
      </div>
      ...
      <div style={{ flex: 1, 'min-width': 0, background: '#FFFFFF', display: 'flex', 'flex-direction': 'column' }}>
        {/* Header */}
        <div style={{ padding: '30px 40px 22px', 'border-bottom': '1px solid #E5E5E5' }}>
          <div style={{ 'font-size': '32px', 'font-weight': 500, 'letter-spacing': '-0.03em', color: '#1A1A1A', 'line-height': 1.1 }}>
            {activeNav() === 'accounts' ? 'Accounts' : SETTINGS_NAV.find((n) => n.id === activeNav())?.name}
          </div>
        ...
        {/* Body */}
        <div style={{ flex: 1, overflow: 'auto', padding: '26px 40px', display: 'flex', 'flex-direction': 'column', gap: '16px' }}>
          <Show when={activeNav() === 'accounts'}>
            <For each={props.accounts}>
              {(accItem) => (
                <div style={{ border: '1px solid #E5E5E5', 'border-radius': '11px', overflow: 'hidden' }}>
                  {/* Account Bar */}
                  <div style={{ display: 'flex', 'align-items': 'center', gap: '14px', padding: '16px 18px', background: '#FBFBFB' }}>
                    <div
                      style={{
                        width: '34px',
                        height: '34px',
                    ...
                    <div style={{ display: 'flex', 'flex-direction': 'column', gap: '2px' }}>
                      <span style={{ 'font-size': '14.5px', 'font-weight': 500, 'letter-spacing': '-0.01em' }}>
                        {accItem.account.displayName}
                      </span>
                      <span style={{ 'font-family': 'var(--al-font-mono)', 'font-size': '10.5px', color: '#888888' }}>
                    ...
                    <div style={{ display: 'flex', 'align-items': 'center', gap: '7px' }}>
                      <div
                        style={{
                          ...
                          height: '7px',
                          'border-radius': '50%',
                          background: accItem.account.status === 'error' ? 'var(--al-cal-classes, #C2410C)' : 'var(--al-cal-work, #0F766E)',
                        }}
                      />
                      <span style={{ 'font-family': 'var(--al-font-mono)', 'font-size': '10.5px', color: '#888888' }}>
                    ...
                    <For each={accItem.calendars}>
                      {(cal) => (
                        ...
                        >
                          ...
                          />
                          <span style={{ 'font-size': '12.5px', color: cal.enabled ? cal.color : '#888888' }}>
                            {cal.name}
                          </span>
                          <input
                            type="checkbox"
                            checked={cal.enabled}
                            onChange={(e) => props.onToggleCalendar(cal.id, e.currentTarget.checked)}
            ...
            <div style={{ display: 'flex', 'align-items': 'center', gap: '18px', 'padding-top': '6px' }}>
              <span style={{ 'font-family': 'var(--al-font-mono)', 'font-size': '10px', 'letter-spacing': '0.08em', color: '#A0A0A0' }}>
                SYNC EVERY
              </span>
              <div style={{ display: 'flex', 'align-items': 'center', gap: '2px', padding: '3px', background: '#EDEDED', 'border-radius': '9px' }}>
                {[5, 15, 60, 0].map((mins) => {
              ...
              <span style={{ 'font-family': 'var(--al-font-mono)', 'font-size': '10.5px', color: '#A0A0A0' }}>
          ...
          <Show when={activeNav() !== 'accounts'}>
            <div style={{ padding: '24px', 'font-size': '13.5px', color: '#777777', background: '#FBFBFB', 'border-radius': '8px' }}>
              Settings for <strong>{SETTINGS_NAV.find((n) => n.id === activeNav())?.name}</strong> are synchronized across devices.
            </div>
          </Show>
        </div>
      </div>
